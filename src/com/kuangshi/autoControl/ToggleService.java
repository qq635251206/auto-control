...
public class ToggleService extends Service {
	...
	void handleStart(Intent intent, int startId) {
		...
		Log.d(TAG, "Added "+component+" to service queue, now size: "+mStates.size());
	...
	synchronized void processNextItem() {
		...
		if (component == null) {
			...
			Log.d(TAG, "ToggleService mQueue empty, shutting down");
			stopSelf();
			Log.d(TAG, "Closing all root shells");
			try {
		...
		Log.d(TAG, "Processing "+component+", remaining items in queue: "+mStates.size());
			...
			public void run() {
				...
				mHandler.post(new Runnable() {
					public void run() {
						...
						Log.d(TAG, "Processing "+component+" done");
						onQueueModified(component, false);
						processNextItem();
					}
				});
