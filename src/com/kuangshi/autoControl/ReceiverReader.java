...
import com.kuangshi.autoControl.bean.ComponentInfo;
import com.kuangshi.autoControl.bean.IntentFilterInfo;
import com.kuangshi.autoControl.bean.PackageInfo;
...
public class ReceiverReader {
	...
	public ArrayList<IntentFilterInfo> load() {
		...
		for (int i=0; i<packageCount; i++)
		{
			...
			LogUtils.v(TAG, "Processing package "+p.packageName);
			parsePackage(p);
	...
	void startIntentFilter() {
		...
		if (priorityRaw != null)
			try {
				...
				LogUtils.w(TAG, "Unable to parse priority value "+
			...
			if (mCurrentFilterPriority != 0)
				LogUtils.v(TAG, "Receiver "+mCurrentComponent.componentName+
	...
	void startAction() {
		...
		if (action == null || !action.equals(Intent.ACTION_BOOT_COMPLETED)) {
			LogUtils.w(TAG, "Receiver "+mCurrentComponent.componentName+
	...
	private String resolveValue(String in, Resources r) {
		...
		try {
			...
			LogUtils.e(TAG, "Unable to resolve resource "+in, e);
			return in;
		}
